...
public class BaseDao {
	...
	public Map<String,Object> queryForObject(String sql ,List<Object> params){
		//这里new出来一个集合对象，这个集合中只能存储Map<String,Object>
	 ...
	 * @return
	 */
	public int excuteUpdate(String sql,PageData pd,String colNames){
		Connection conn = null;
		PreparedStatement pstm = null;
		int result = 0;
		try{
			...
			for(int i = 0 ; i <cols.length ;i++){
				pstm.setObject(i+1, pd.get(cols[i]));
			}
			result = pstm.executeUpdate();
		}catch(Exception ex){
			ex.printStackTrace();
